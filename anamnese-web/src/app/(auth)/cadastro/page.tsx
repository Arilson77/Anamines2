    ...
    <div className="min-h-screen flex items-center justify-center bg-stone-50">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-stone-200 p-8">
        ...
        <form onSubmit={handleCadastro} className="space-y-4">
          {[
            { campo: 'nome',      label: 'Nome do consultório', type: 'text',     placeholder: 'Dra. Ana Souza',  autoComplete: 'organization' },
            { campo: 'email',     label: 'E-mail',              type: 'email',    placeholder: 'seu@email.com',   autoComplete: 'email' },
            { campo: 'senha',     label: 'Senha',               type: 'password', placeholder: '••••••••',        autoComplete: 'new-password' },
            { campo: 'confirmar', label: 'Confirmar senha',      type: 'password', placeholder: '••••••••',        autoComplete: 'new-password' },
          ].map(({ campo, label, type, placeholder, autoComplete }) => (
            <div key={campo}>
              <label htmlFor={campo} className="block text-xs font-medium text-stone-500 uppercase tracking-wide mb-1">{label}</label>
              <input id={campo} name={campo} type={type} autoComplete={autoComplete}
