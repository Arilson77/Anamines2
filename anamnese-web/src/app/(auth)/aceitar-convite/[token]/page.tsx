...
export default function AceitarConvitePage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      router.push('/login?convite=1');
    } catch (err: unknown) {
      setErro(err instanceof Error ? err.message : 'Convite inválido ou expirado.');
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-stone-50">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-stone-200 p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-stone-400 mt-1">Você foi convidado para o Anamnese</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-stone-500 uppercase tracking-wide mb-1">Seu nome</label>
